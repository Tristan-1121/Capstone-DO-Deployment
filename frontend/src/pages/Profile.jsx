...
export default function Profile() {
  ...
  if (loading)
    return <div className="p-6 text-gray-500">Loading patient info...</div>;
    ...
    <div className="p-6 space-y-6">
      ...
      <section className="bg-white rounded border p-4 shadow-sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-y-2">
          ...
          <div>
            <strong>Height:</strong> {formatHeight(patient.Height)}
          </div>
          <div>
      ...
      {/* === Health / Allergies / Prescriptions === */}
      <section className="bg-white rounded border p-4 shadow-sm">
        <h3 className="font-semibold text-lg mb-3">Health History</h3>
          {patient.MedHist?.length ? (
            <ul className="list-disc ml-6 space-y-2">
              {patient.MedHist.map((h, i) => (
                <li key={i}>
                  ...
                  <div><strong>Surgeries:</strong> {h.surgeries?.join(", ") || "—"}</div>
            ...
            <p className="text-gray-500">No history on file.</p>
          )}
      </section>
      ...
      <section className="bg-white rounded border p-4 shadow-sm">
        ...
        {patient.Allergies?.length ? (
          ...
          <p className="text-gray-500">No allergies on file.</p>
        )}
      </section>
